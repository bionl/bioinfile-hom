...
workflow {
    ...
    all_samples.fasta.mix(CONVERT_FASTQ_TO_FASTA.out)
        | FILTER_QUALITY
        | countFasta
        | view {
            def newFile = file("gs://bionfile-test/counts/${workflow.runName}.txt")
            newFile.text = it
        }

    RUN_BLAST(FILTER_QUALITY.out, 'gs://bionfile-test/hom/db')

    MAP_TAXONOMIES(RUN_BLAST.out, 'gs://bionfile-test/hom/HOM16S.taxonomy.tsv')

    DECISION_MAKING(MAP_TAXONOMIES.out, 'gs://bionfile-test/blastdm.py').collect() | MERGE_FEATURES
}
